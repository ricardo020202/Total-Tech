...
module.exports = class Dieta {
    ...
    static fetchByCal(numcal, start)
    {
        ...
        {
            return db.execute('SELECT * FROM dieta WHERE no_calorias = ? ORDER BY no_calorias  LIMIT ?, 9', [numcal, start]);
        }

        if(numcal >= 4000)
        {
            return db.execute('SELECT * FROM dieta WHERE no_calorias BETWEEN ? AND ? + 1000 ORDER BY no_calorias  LIMIT ?, 9', [numcal, numcal, start]);
        }

        if(numcal <= 900)
        {
            return db.execute('SELECT * FROM dieta WHERE no_calorias BETWEEN ? AND ? - 500 ORDER BY no_calorias  LIMIT ?, 9', [numcal, numcal, start]);
        }
    ...
    update() {
        return db.execute(
            'UPDATE dieta SET nombre_dieta = ?, no_calorias = ?, proteinas = ?, carbohidratos = ?, grasas = ?, fibra_total = ?, ceniza = ?, calcio = ?, fosforo = ?, hierro = ?, tiamina = ?, riboflavina = ?, niacina = ?, vitamina_c = ?, vitamina_a = ?, ac_graso_mono = ?, ac_graso_poli = ?, ac_graso_saturado = ?, colesterol = ?, potasio = ?, sodio = ?, zinc = ?, magnesio = ?, vit_b6 = ?, vit_b12 = ?, ac_folico = ?, folato = ? WHERE id_dieta = ?',
